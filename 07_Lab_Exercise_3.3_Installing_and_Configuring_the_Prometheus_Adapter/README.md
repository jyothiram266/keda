...
1. Configure the Prometheus Adapter:
...
prometheus-adapter.yaml file with following contents:
...
custom:
- seriesQuery:
'http_requests_total{namespace="default",job="monitoring/go-http-server"}'
resources:
...
metricsQuery: 'sum(rate(http_requests_total{<<.LabelMatchers>>}[1m])) by
(<<.GroupBy>>)'
...
custom metric. Here,
http_requests_total{namespace="default",job="monitoring/go-http-server
"} selects the http_requests_total metric from the monitoring/go-http-server job
...
i. namespace: {resource: "namespace"}: Maps the namespace label in
Prometheus to the namespace resource in Kubernetes.
ii. pod: {resource: "pod"}: Maps the pod label in Prometheus to the pod resource in
Kubernetes.
...
aggregate metrics data. sum(rate(requests_total{<<.LabelMatchers>>}[1m])) by
(<<.GroupBy>>), calculates the sum of the rate of the http_requests_total metric over a
1-minute window, grouped by the labels specified in the <<.GroupBy>> placeholder.
...
kubectl get --raw
```
"/apis/custom.metrics.k8s.io/v1beta1/namespaces/default/pods/*/http_requests_per_
second?labelSelector=app=go-http-server" | jq
{
